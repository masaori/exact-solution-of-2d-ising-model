# ---------------------------------------------------------
# SageMath: 式1(L.10)と式2(L.11)の等価性検証コード
# ---------------------------------------------------------
...
# 第1項
term1_num = (exp(2*K1) + 1) - (exp(2*K1)*exp(-2*K2) - exp(-2*K2))*exp(i*theta_mu)
term1_den = (exp(2*K1) + 1) - (exp(2*K1)*exp(-2*K2) - exp(-2*K2))*exp(-i*theta_mu)
term1 = term1_num / term1_den

# 第2項
term2_num = exp(2*K1)*exp(-2*K2) + exp(-2*K2) - (exp(2*K1) - 1)*exp(i*theta_mu)
term2_den = exp(2*K1)*exp(-2*K2) + exp(-2*K2) - (exp(2*K1) - 1)*exp(-i*theta_mu)
term2 = term2_num / term2_den

# 式1 = term1 * term2（sqrtは取らない）
rad1 = term1 * term2

# 簡約
rad1_simplified = rad1.simplify_full()

print("\n--- rad1 (L.10) simplified ---")
...
num2 = (
    ((exp(2*K1) + 1) - (exp(2*K1)*exp(-2*K2) - exp(-2*K2))*exp(i*theta_mu))
    * ((exp(2*K1)*exp(-2*K2) + exp(-2*K2)) - (exp(2*K1) - 1)*exp(i*theta_mu))
)
...
print("\n--- rad2 (L.11) simplified ---")
print(rad2_simplified)

# ---------------------------------------------------------
# 記号的な等価性チェック
# ---------------------------------------------------------
print("\n--- Symbolic check of expressions ---")
