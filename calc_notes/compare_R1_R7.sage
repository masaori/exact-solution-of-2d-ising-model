# ---------------------------------------------------------
# SageMath: 式1 (R.1) と式2 (R.7) の等価性検証コード
# ---------------------------------------------------------
...
rad1 = (
    i * exp(i*theta_mu) * s_2_star * (c_1*cos(theta_mu) - i*sin(theta_mu) - s_1*c_2)
) / (
    i * exp(-i*theta_mu) * s_2_star * (c_1*cos(-theta_mu) - i*sin(-theta_mu) - s_1*c_2)
)
...
#       + (e^(2 K_1) + e^(-2 K_1) - 1) e^(2 sqrt(-1) theta_(mu))
#     )
#     /
#     (
#       (e^(2 K_1) + e^(-2 K_1) + 1)
#       - (e^(2 K_1) - e^(-2 K_1)) (e^(2 K_2) + e^(-2 K_2)) e^(-sqrt(-1) theta_(mu))
#       + (e^(2 K_1) + e^(-2 K_1) - 1) e^(-2sqrt(-1) theta_(mu))
#     )
...
# ---------------------------------------------------------
# 数値代入による確認（radicandを直接比較）
# ---------------------------------------------------------
print("\n--- Numerical Verification (theta_mu grid, comparing radicands directly) ---")
M_val = 100
