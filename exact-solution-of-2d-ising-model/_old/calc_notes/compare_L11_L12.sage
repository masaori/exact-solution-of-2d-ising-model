...
num1 = (
    (
        (exp(2*K1) + 1)
        -
        (exp(2*K1)*exp(-2*K2) - exp(-2*K2))*exp(i*theta_mu)
    )
    *
    (
        (exp(2*K1)*exp(-2*K2) + exp(-2*K2))
        -
        (exp(2*K1) - 1)*exp(i*theta_mu)
    )
)
...
if all_ok:
    print("Result: All numerical checks (radicands without sqrt) passed within precision limits.")
