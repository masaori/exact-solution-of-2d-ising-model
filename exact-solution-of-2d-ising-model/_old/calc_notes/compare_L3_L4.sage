# ---------------------------------------------------------
# SageMath: 式1 (L.3) と式2 (L.4) の等価性検証コード
...
rad1 = (
    (1 - (tanh(K1) * tanh(K2_star)) * exp(i*theta_mu))
    /
    (1 - (tanh(K1) * tanh(K2_star)) * exp(-i*theta_mu))
    *
    (1 - (tanh(K1) * tanh(K2_star)^(-1)) * exp(i*theta_mu))
    /
    (1 - (tanh(K1) * tanh(K2_star)^(-1)) * exp(-i*theta_mu))
)
...
rad2 = (
    ...
    (1 - (tanh(K1) * tanh(K2_star)) * exp(-i*theta_mu))
    *
    (tanh(K2_star) - tanh(K1) * exp(i*theta_mu))
    /
    (tanh(K2_star) - tanh(K1) * exp(-i*theta_mu))
)
...
# 記号的な等価性チェック
# ---------------------------------------------------------
print("\n--- Symbolic check of expressions (without sqrt) ---")
